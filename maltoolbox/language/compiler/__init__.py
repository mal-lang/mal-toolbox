...
class MalCompiler(ParseTreeVisitor):
    ...
    def visitStep(self, ctx):
        ...
        if ctx.attackstep():
        ...
        elif ctx.notexiststep():
            ctx = ctx.notexiststep()
            step['type'] = 'notExist'
            step['requires'] = (
                self.visit(ctx.precondition()) if ctx.precondition() else None
            )
            ...
            # error
            raise Exception('Unrecognized step type ' + str(ctx.getText()))
