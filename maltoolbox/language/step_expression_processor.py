...
def process_set_operation_step_expression(
    ...
    rh_target_asset, rh_expr_chain, _ = process_step_expression(
        assets, target_asset, expr_chain, step_expression['rhs'], lang_spec
    )

    if lh_target_asset is None:
        raise ValueError(f'No lh target in step expression {step_expression}')
    if rh_target_asset is None:
        raise ValueError(f'No rh target in step expression {step_expression}')
...
def process_variable_step_expression(
    ...
    var_target_asset, var_expr_chain = resolve_variable(
        assets, target_asset, var_name, lang_spec
    )
...
def process_field_step_expression(
    ...
    if target_asset is None:
        raise ValueError(f'Missing target asset for field "{fieldname}"!')
    ...
    for association in target_asset.associations.values():
        if (
            ...
            return (
                ...
                ExpressionsChain(
                    type=ExprType.FIELD, fieldname=fieldname, association=association
                ),
                None,
            )
...
def process_transitive_step_expression(
    ...
    """
    result_target_asset, result_expr_chain, _ = process_step_expression(
        assets, target_asset, expr_chain, step_expression['stepExpression'], lang_spec
    )
    new_expr_chain = ExpressionsChain(
        type=ExprType.TRANSITIVE, sub_link=result_expr_chain
    )
...
def process_collect_step_expression(
    ...
    if lh_target_asset is None:
        raise ValueError(
            f'No left hand asset in collect expression {step_expression["lhs"]}'
        )

    rh_target_asset, rh_expr_chain, rh_attack_step_name = process_step_expression(
    ...
    if rh_expr_chain:
        new_expr_chain = ExpressionsChain(
            type=ExprType.COLLECT, left_link=lh_expr_chain, right_link=rh_expr_chain
        )
...
def process_step_expression(
    ...
    if step_expression['type'] == 'attackStep':
        result = process_attack_step_expression(target_asset, step_expression)
        ...
        result = process_field_step_expression(target_asset, step_expression)
        ...
        raise LookupError(f'Unknown attack step type: "{step_expression["type"]}"')
    return result


def reverse_expr_chain(
    expr_chain: ExpressionsChain | None, reverse_chain: ExpressionsChain | None
) -> ExpressionsChain | None:
    ...
    if expr_chain.type.is_binary():
        left_reverse_chain = reverse_expr_chain(expr_chain.left_link, reverse_chain)
        right_reverse_chain = reverse_expr_chain(expr_chain.right_link, reverse_chain)
    ...
    if expr_chain.type == ExprType.TRANSITIVE:
        result_reverse_chain = reverse_expr_chain(expr_chain.sub_link, reverse_chain)
        new_expr_chain = ExpressionsChain(
            type=ExprType.TRANSITIVE, sub_link=result_reverse_chain
        )
        return new_expr_chain
    ...
    if expr_chain.type == ExprType.FIELD:
        ...
        if not association:
            raise LanguageGraphException('Missing association for expressions chain')

        if not expr_chain.fieldname:
            raise LanguageGraphException('Missing field name for expressions chain')

        opposite_fieldname = association.get_opposite_fieldname(expr_chain.fieldname)
        new_expr_chain = ExpressionsChain(
            type=ExprType.FIELD, association=association, fieldname=opposite_fieldname
        )
        return new_expr_chain
