            ...
            for asset_assoc_field, associated_assets in asset.associated_assets.items():
                ...
                for assoc_asset in associated_assets:
                    ...
                    if target_asset:
                        ...
                        source_x = (
                            asset.extras.get('position', {}).get('x', 0) + box_width / 2
                        )
                        source_y = (
                            asset.extras.get('position', {}).get('y', 0)
                            + box_height / 2
                        )
                        target_x = (
                            target_asset.extras.get('position', {}).get('x', 0)
                            + box_width / 2
                        )
                        target_y = (
                            target_asset.extras.get('position', {}).get('y', 0)
                            + box_height / 2
                        )
