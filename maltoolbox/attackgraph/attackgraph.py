...
class AttackGraph:
    ...
    def _follow_expr_chain(
        ...
        match expr_chain.type:
            ...
            case 'collect':
                if not expr_chain.left_link:
                    raise LanguageGraphException(
                        '"collect" step expression chain' ' is missing the left link.'
                    )
                if not expr_chain.right_link:
                    raise LanguageGraphException(
                        '"collect" step expression chain' ' is missing the right link.'
                    )
                lh_targets = self._follow_expr_chain(
                    model, target_assets, expr_chain.left_link
                )
                ...
                for lh_target in lh_targets:
                    rh_targets |= self._follow_expr_chain(
                        model, {lh_target}, expr_chain.right_link
                    )
                return rh_targets
            ...
            for attack_step in asset.lg_asset.attack_steps.values():
                ...
                match attack_step.type:
                    ...
                    case 'exist' | 'notExist':
                        ...
                        logger.debug(
                            'Setting the existence status of "%s" to ' '%s.',
                            node_name,
                            existence_status,
                        )
    ...
    def add_node(
        ...
        if logger.isEnabledFor(logging.DEBUG):
            # Avoid running json.dumps when not in debug
            logger.debug(
                'Create and add to attackgraph node of type "%s" '
