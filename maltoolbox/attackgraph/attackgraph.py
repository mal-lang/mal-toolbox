...
class AttackGraph:
    ...
    def __init__(self, lang_graph, model: Model | None = None) -> None:
    ...
    def _to_dict(self) -> dict:
        ...
        logger.debug(
            'Serialized %d attack steps and %d attackers.'
    ...
    def _from_dict(
        ...
        for node_dict in serialized_attack_steps.values():
            ...
            ag_node.tags = set(node_dict['tags']) if 'tags' in node_dict else set()
        ...
        for node_dict in serialized_attack_steps.values():
            ...
            if not isinstance(_ag_node, AttackGraphNode):
                msg = (
                    'Failed to find node with id %s when loading attack graph from dict'
                )
    ...
    def load_from_file(
        ...
        return cls._from_dict(serialized_attack_graph, lang_graph, model=model)
    ...
    def _follow_expr_chain(
        self, model: Model, target_assets: set[Any], expr_chain: ExpressionsChain | None
    ) -> set[Any]:
        ...
        match expr_chain.type:
            case 'union' | 'intersection' | 'difference':
                ...
                if not expr_chain.left_link:
                    msg = (
                        f'"{expr_chain.type}" step expression chain'
            ...
            case 'field':
                ...
                if not expr_chain.fieldname:
                    msg = '"field" step expression chain is missing fieldname.'
                    raise LanguageGraphException(msg)
            ...
            case 'transitive':
                if not expr_chain.sub_link:
                    msg = '"transitive" step expression chain is missing sub link.'
                    raise LanguageGraphException(msg)
            ...
            case 'subType':
                if not expr_chain.sub_link:
                    msg = '"subType" step expression chain is missing sub link.'
                    raise LanguageGraphException(msg)
                new_target_assets = set()
                new_target_assets.update(
                    self._follow_expr_chain(model, target_assets, expr_chain.sub_link)
                )
                ...
                for asset in new_target_assets:
                    ...
                    if not lang_graph_subtype_asset:
                        msg = (
                            f'Failed to find asset "{expr_chain.subtype}" in the '
                            'language graph.'
                        )
                        raise LookupError(msg)
            ...
            case 'collect':
                if not expr_chain.left_link:
                    msg = '"collect" step expression chain is missing the left link.'
                    raise LanguageGraphException(msg)
                if not expr_chain.right_link:
                    msg = '"collect" step expression chain is missing the right link.'
                    raise LanguageGraphException(msg)
                lh_targets = self._follow_expr_chain(
                    model, target_assets, expr_chain.left_link
                )
                return self._follow_expr_chain(model, lh_targets, expr_chain.right_link)
    ...
    def _generate_graph(self) -> None:
        ...
        for asset in self.model.assets:
            ...
            for attack_step in lang_graph_asset.attack_steps.values():
                logger.debug('Generating attack step node for %s.', attack_step.name)
        ...
        for ag_node in self.nodes:
            ...
            if not ag_node.asset:
                msg = 'Attack graph node is missing asset link'
                raise AttackGraphException(msg)
            lang_graph_asset = self.lang_graph.assets[ag_node.asset.type]

            lang_graph_attack_step = lang_graph_asset.attack_steps[ag_node.name]
            ...
            while lang_graph_attack_step:
                for child in lang_graph_attack_step.children.values():
                    for target_attack_step, expr_chain in child:
                        target_assets = self._follow_expr_chain(
                            self.model, {ag_node.asset}, expr_chain
                        )
    ...
    def add_node(self, node: AttackGraphNode, node_id: int | None = None) -> None:
        ...
        if node.id in self._id_to_node:
            msg = f'Node index {node_id} already in use.'
            raise ValueError(msg)
    ...
    def add_attacker(
        ...
        reached_attack_steps: list[int] | None = None,
    ) -> None:
        ...
        """
        if reached_attack_steps is None:
            reached_attack_steps = []
        ...
        if attacker.id in self._id_to_attacker:
            msg = f'Attacker index {attacker_id} already in use.'
            raise ValueError(msg)
        ...
        for node_id in reached_attack_steps:
            ...
            if node:
                ...
                msg = 'Could not find node with id %din reached attack steps.'
        ...
        for node_id in entry_points:
            ...
            if node:
                ...
                msg = 'Could not find node with id %din attacker entrypoints.'
                logger.error(msg, node_id)
                raise AttackGraphException(msg % node_id)
        self.attackers.append(attacker)
        self._id_to_attacker[attacker.id] = attacker

    def remove_attacker(self, attacker: Attacker) -> None:
