...
from maltoolbox.model import AttackerAttachment, Model, ModelAsset
...
def test_model_remove_nonexisting_asset(model: Model):
    ...
    asset1 = ModelAsset(
        name='TestAsset', asset_id=1, lg_asset=model.lang_graph.assets['Application']
    )
...
def test_model_add_attacker(model: Model):
    ...
    attacker2.entry_points = [(testcreds, attack_steps)]
    model.add_attacker(attacker2, attacker_id=attacker2_id)
...
def test_model_attacker_to_dict(model: Model):
    ...
    attack_steps = ['attemptCredentialsReuse']
    attacker.entry_points = [(asset1, attack_steps)]
